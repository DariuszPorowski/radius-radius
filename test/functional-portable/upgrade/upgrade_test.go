...
// It uses TryNewRPTestOptions (which returns errors instead of calling t.FailNow)
// because require.Eventually runs the condition in a goroutine. Calling t.FailNow
// from a non-test goroutine triggers runtime.Goexit which cannot be caught by
// recover(), silently killing the goroutine and permanently breaking the retry loop.
func waitForControlPlane(t *testing.T, ctx context.Context) rp.RPTestOptions {
	t.Helper()
	var options rp.RPTestOptions
	require.Eventually(t, func() bool {
		opts, err := rp.TryNewRPTestOptions(t)
		if err != nil {
			t.Logf("Control plane not ready yet: %v", err)
			return false
		}
		options = opts
