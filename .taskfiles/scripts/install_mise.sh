...
readonly INSTALL_SCRIPT_URL="https://mise.run"
...
# Normalize VERSION: empty/whitespace -> "", numeric -> add "v" prefix
# Note: Mise expects empty string for latest, not "latest" keyword
...
if ! curl -fsSL --proto '=https' --tlsv1.3 "${INSTALL_SCRIPT_URL}" | MISE_VERSION="${VERSION}" MISE_INSTALL_DIR="${INSTALL_DIR}" sh -s -- "${VERSION}"; then
